#pragma once 
#include <BP_UnderlineTabBtn_Emoticon_Temp_Structs.h>
 
 
 
class UBP_UnderlineTabBtn_Emoticon_Temp_C
{
public:
	UBP_UnderlineTabBtn_Emoticon_Temp_C(std::uint64_t p_addr = 0) : m_addr(p_addr) {}
	...
	struct UFocusAnimSet_Underline_C GetFocusAnimSet_Underline() {
		auto ptr_addr = memory.read<std::uint64_t>(m_addr + 1024);
		return struct UFocusAnimSet_Underline_C(ptr_addr);
	}
