...
class UBP_EventMainTab_Button_C
{
public:
	UBP_EventMainTab_Button_C(std::uint64_t p_addr = 0) : m_addr(p_addr) {}
	...
	struct UKdTextBlock GetV_TitleName() {
		auto ptr_addr = memory.read<std::uint64_t>(m_addr + 1040);
		return struct UKdTextBlock(ptr_addr);
	}
	struct FText GetInst_TabName() {
